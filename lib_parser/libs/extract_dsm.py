...
def load_altay_data(execution_date: datetime = None):
    ...
    oracle_query = f"""SELECT * from DATA_MART."V$ALTAY_DATA" where STAT_DATE between {start_date} and {end_date}"""

    ch_query = f"""select * from stg.mart_dcm_data on cluster cluster_2S_2R where deleted_flag = 0"""
    ...
    try:
        ...
        if not df.empty:
            ...
            df_insert_del_rows = (ch_client.merge(
                                                df_ch, 
                                            ...
                                            .assign(
                                                ...
                                                deleted_flag=1,
                                                hash_diff=lambda df: df.apply(compute_row_hash, axis=1)  # Хеш для всех столбцов
